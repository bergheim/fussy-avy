...
(use-package fussy-avy
  ...
  :config
  ;; For Evil users: must be in :config to ensure fussy-avy is loaded first
  ;; (with-eval-after-load 'evil
  ;;   (define-key evil-normal-state-map (kbd "g ") #'evil-fussy-avy-goto-char-timer))
  ;; (setq fussy-avy-max-distance 2)      ; Max edit distance to tolerate
...
;; For Evil users: wrap in with-eval-after-load to handle load order
;; (with-eval-after-load 'evil
;;   (define-key evil-normal-state-map (kbd "g ") #'evil-fussy-avy-goto-char-timer))
...
This installs avy to =.deps/= and runs the ERT test suite (32 tests).
